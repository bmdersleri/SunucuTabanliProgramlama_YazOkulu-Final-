...
if(get('id')){
	...
	$query = query("select * from blog_content where blogid= " . $edit_id . " and lang=". $lang);
	...
	while($langFetch = fetch($langQuery)){
		...
		$languages .= '<li class="nav-item"><a href="'. admin_url('blog_edit?id=' . $edit_id . '&lang='. $langFetch['id']) .'" class="nav-link' .
...
require controller("admin/blog_add");
