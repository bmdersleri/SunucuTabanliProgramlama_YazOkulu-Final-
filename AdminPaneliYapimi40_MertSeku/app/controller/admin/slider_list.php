...
if(get('delete')){
	query("delete from slider where id=" . get('delete'));
	query("delete from slider_content where sliderid=" . get('delete'));
...
$sqlText = "select * from slider as b, slider_content as bc where bc.sliderid = b.id and bc.lang = 1 {$search} order by bc.sort asc";
...
while($f = fetch($query)){
	$contents = "";
	$categoryLang = query("select DISTINCT lang from slider_content where sliderid=" . $f['sliderid']);
	...
	<tr class="text-muted">
		...
		<td>
			<span>
				<a href="' . admin_url('slider_edit?id='.$f['sliderid']) . '" class="mr-4" data-toggle="tooltip" data-placement="top" title="Edit">
					<i class="fa fa-pencil color-muted"></i>
				</a>
				<a href="' . admin_url('slider_list?delete='.$f['sliderid']) . '" onclick="return confirm(\'Are you sure you want to delete this slider?\')' .'" title="Close">
...
$InfoArray = $Paging->InfoArray();
	...
	if($InfoArray["PREV_PAGE"]) {
		$pages .= '<li class="page-item"><a class="page-link" href="' . admin_url('slider_list') . '">❰❰</a></li>';
		$pages .= '<li class="page-item"><a class="page-link" href="' . admin_url('/slider_list?page='. $InfoArray["PREV_PAGE"]) . '">❰</a></li>';
	}else{
	...
	for($i=0; $i<count($InfoArray["PAGE_NUMBERS"]); $i++) {
		if(ceil($InfoArray["CURRENT_PAGE"]) == $InfoArray["PAGE_NUMBERS"][$i]) {
			...
			$pages .= '<li class="page-item"><a href="'.admin_url('slider_list?page='.$InfoArray["PAGE_NUMBERS"][$i]).'" class="page-link">'. $InfoArray["PAGE_NUMBERS"][$i].'</a></li>';
		}
	}
	if($InfoArray["NEXT_PAGE"]) {
		$pages .= '<li class="page-item"><a href="'.admin_url('slider_list?page='.$InfoArray["NEXT_PAGE"]).'" class="page-link">❱</a></li>';
		$pages .= '<li class="page-item"><a href="'.admin_url('slider_list?page='. $total_page).'" class="page-link">❱❱</a></li>';
	}else{
		$pages .= '<li class="page-item"><a class="page-link">❱</a></li>';
		$pages .= '<li class="page-item"><a class="page-link">❱❱</a></li>';
	}
...
require view("admin/slider_list");
