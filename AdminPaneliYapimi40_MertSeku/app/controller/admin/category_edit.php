...
if(get('id')){
	...
	$query = query("select * from pcategory_content where categoryid= " . $edit_id . " and lang=". $lang);
	...
	while($langFetch = fetch($langQuery)){
		...
		$languages .= '<li class="nav-item"><a href="'. admin_url('category_edit?id=' . $edit_id . '&lang='. $langFetch['id']) .'" class="nav-link' .
...
require controller("admin/category_add");
