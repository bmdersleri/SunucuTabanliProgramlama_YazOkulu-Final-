...
class VT 
{
  ...
  var $user="root"; 
  var $password="3738mhs.123456789";
  ...
  public function ModulEkle()
  {
	...
	public function kategoriGetir($tablo,$secID="",$uzanti="-1")
	{
		$uzanti++;
		...
		if($kategori!=false)
		{
			for($q=0;$q<count($kategori);$q++)
			{
				...
				if($secID==$kategoriID)
				{
					echo '<option value="'.$kategoriID.'" selected="selected">'.str_repeat("&nbsp;nbsp;&nbsp;",$uzanti).stripslashes($kategori[$q]["baslik"]).'</option>';
					
				}
				else
				{
					echo '<option value="'.$kategoriID.'">'.str_repeat("&nbsp;nbsp;&nbsp;",$uzanti).stripslashes($kategori[$q]["baslik"]).'</option>';
					
				}
				if($kategoriseflink==$tablo){break;}
				$this->kategoriGetir($kategoriseflink,$secID,$uzanti);
			}
		}
		else
		{
	...
	public function tekKategori($tablo,$secID="",$uzanti="-1")
	{
		$uzanti++;
