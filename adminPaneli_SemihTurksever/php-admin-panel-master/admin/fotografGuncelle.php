...
$sorgu = $baglanti->prepare("SELECT * FROM fotograf Where id=:id");
...
$sonuc = $sorgu->fetch();//sorgu çalıştırılıp veriler alınıyor
       ...
       if ($_POST) { // Post olup olmadığını kontrol ediyoruz.
            ...
            if ($baslik <> "" && $icerik <> "" && $hata == "") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
             ...
             $sql = "UPDATE fotograf SET foto=:foto, baslik=:baslik,aktif=:aktif,sira=:sira, ustBaslik=:ustBaslik, icerik=:icerik WHERE id=:id;";             
             $durum = $baglanti->prepare($sql)->execute($satir);

             if ($durum)
             {
                echo '<script>swal("Başarılı","Ürün güncellendi","success").then((value)=>{ window.location.href = "fotograf.php"});
