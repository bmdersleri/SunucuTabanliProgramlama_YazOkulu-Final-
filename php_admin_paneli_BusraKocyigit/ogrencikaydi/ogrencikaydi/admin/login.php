...
if(isset($_SESSION['user_login'])){
	...
	if (isset($_POST['login'])) {
		...
		if (empty($username)) {
			$input_arr['input_user_error']= "Kullanıcı Adı Gerekli!";
		}

		if (empty($password)) {
			$input_arr['input_pass_error']= "Şifre Gerekli!";
		}
			...
			if (mysqli_num_rows($result)==1) {
				...
				if ($row['password']==sha1(md5($password))) {
					if ($row['status']=='active') {
						...
						$status_inactive = "Durumunuz aktif değil,
						lütfen yönetici ile iletişime geçin!";
					}
				}else{
					$worngpass= "Hatalı Şifre Girdiniz!";	
				}
			}else{
				$usernameerr= " Böyle Bir Kullanıcı Adı Bulunamadı!";
			}
		}
...
<html lang="en">
  ...
  <body>
    <div class="container"><br>
          <h1 class="text-center">Yönetici Girişi</h1><hr><br>
            ...
            <div class="col-md-4 offset-md-4">
             	<form method="POST" action="">
				  ...
				  <div class="text-center">
				      <button type="submit" name="login" class="btn btn-warning">Giriş Yap</button>
				    </div>
				    <p>Eğer kullanıcı hesabınız yoksa, <a href="register.php"> şimdi kaydolun!</a></p>
				  </div>
				</form>
            </div>
          </div>
