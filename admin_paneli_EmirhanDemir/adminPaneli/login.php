    ...
    <div class="card-body login-card-body">
      ...
      if($_POST)
      {
        if(!empty($_POST["kullanıcı"]) && !empty($_POST["sifre"]))
        {
          ...
          $kontrol=$VT->VeriGetir("kullanicilar","WHERE kullanıcı=? AND sifre=?",array($kullanıcı,$sifre),"ORDER BY ID ASC",1);
          if($kontrol!=false)
          {
        ...
        {
          echo '<div class="alert-danger"> Boş bıraktığınız kısımları doldurunuz.</div>';
        }
      }
      ?>
      <form action="#" method="post">
        <div class="input-group mb-3">
          <input type="text" class="form-control" name="kullanıcı" placeholder="Kullanıcı Adı">
