	   ...
	   if($_POST)
	   {
		   if(!empty($_POST["kategori"]) && !empty($_POST["baslik"]) && !empty($_POST["anahtar"]))
		   {
			   ...
			   if(!empty($_FILES["resim"]["name"]))
			   {
				   ...
				   if($yukle!=false)
				   {
					   $ekle=$VT->SorguCalistir("INSERT INTO ".$kontrol[0]["tablo"],"SET baslik=?, seflink=?, kategori=?, metin=?, resim=?, anahtar=?, durum=?,  tarih=?",array($baslik,$seflink,$kategori,$metin,$yukle,$anahtar,1,date("Y-m-d")));
				   }
				   else
				   {
					    ?>
			   ...
			   {
				   $ekle=$VT->SorguCalistir("INSERT INTO ".$kontrol[0]["tablo"],"SET baslik=?, seflink=?, kategori=?, metin=?, anahtar=?, durum=?, tarih=?",array($baslik,$seflink,$kategori,$metin,$anahtar,1,date("Y-m-d")));
			   }
