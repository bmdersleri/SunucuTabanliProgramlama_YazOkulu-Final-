...
function settings($code){	
	$result = query("select * from settings where name='$code'");	
	$contentFetch =fetch($result);	
	return $contentFetch['description'];
}

function photo($code){	
	$result = query("select * from photos where name='$code'");	
	$contentFetch =fetch($result);	
	return $contentFetch['src'];
}
