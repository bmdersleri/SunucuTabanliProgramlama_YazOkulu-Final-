<?php
	...
	if(get('yukle')){
		...
		else if(get('yukle') == "photos"){
			$id = get('id');
			if($id){
				$result = image_upload($id . "/[random]", "/assets/img/photos", [1900, 1267], "photos");
				$resultDecode = json_decode($result);
				if(isset($resultDecode->success)) {
					$q = query("update photos SET src='$resultDecode->success' where id= $id");
					echo $result;
				}
			}else{
				$error['error'] = "ID YOK!";
